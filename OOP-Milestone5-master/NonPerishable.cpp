		...
		if (ok()) {
			if (linear == true) {
				...
				if (taxed()) {
					...
					ostr << "Name:" << endl << left << name() << endl << "Sku: " << sku() << endl << "Price: " << setprecision(2) << price() << endl << "Price after tax: N/A" << endl << "Quantity: " << quantity() << endl << "Total Cost: " << setprecision(2) << (cost() * quantity()) << endl;
				}
			}
		}
		else {
