...
namespace ict{
  ...
  //This method is created to set the date and time without typing the same code repeatedly.
  ...
  std::ostream & Date::write(std::ostream & ostr) const{
	  char slash = '/';
	  if (dateOnly() == true) {
		  ostr << m_year << slash << setfill('0') << setw(2) << right << m_mon << slash << setfill('0') << setw(2) << right << m_day;
	  }
	  else {
		  char comma = ',', colon = ':';
		  ostr << m_year << slash << setw(2) << setfill('0') << m_mon << slash << setw(2) << setfill('0') << m_day << comma << ' ' << setfill('0') << setw(2) << m_hour << colon << setfill('0') << setw(2) << m_min;
	  }
